...
(
	...
	/*
		This DIQ looks for CAs/PPs with Estimates (ETC).
		To do so, it first collects DS01 WBS_IDs of the above three types in a cte ToTest.
		It then returns rows from DS03 where:
		1. Some ETC exists AND
		2a. The WBS_ID_WP exists (WP/PP rows) and the WBS_ID_WP exists in ToTest OR
		2b. The WBS_ID_WP does NOT exist (SLPP/CA rows) and the WBS_ID_CA exists in ToTest.
	*/
