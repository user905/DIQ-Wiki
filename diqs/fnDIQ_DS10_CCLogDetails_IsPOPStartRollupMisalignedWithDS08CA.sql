...
(
	...
	/*
		This function looks to see whether CA-level transactions have a pop start in
		alignment with what is in the WADs.

		The function covers a very special case where WADs are at the CA-level, 
		but CC log entries are at the WP level.

		To start, the cte, WADStart, collects the latest WAD by WBS_ID (Ds08.WBS_ID is the CA-level WBS),
		with the POP Start.

		CACLog then gets POP starts by CA WBS ID, which it pulls from AncestryTree_Get.
		
		The third cte, FlagsByCAWBS, then joins the above two result sets by CA WBS ID 
		and compares WAD POP Start to CC Log POP Start.
	...
	*/
	...
	WITH WADStart AS (
		...
		--Join back to AncestryTree, but this time by the *Ancestor's* WBS ID
		--Filter for only items where the ancestor is a CA/SLPP
		SELECT A.WBS_ID
	...
	--Join back to DS10 to get problem rows.
	SELECT 
		L.*
	FROM 
