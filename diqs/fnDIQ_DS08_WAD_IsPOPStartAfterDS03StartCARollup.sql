...
(
	...
	/*
		This function looks for WADs where pop start < the last recorded DS03.SPAE value.

		(It covers a special case where WADs are at the WP level, 
		but ACWP is at the CA level in cost.)

		First, use the CostStart cte to load the last recorded SPAE value by CA WBS ID.

		Then rollup the latest revision WP WADs to the CA level with the last POP start date.
