...
(
	...
	/*
		UPDATE: Nov 2023. DID v5 added an is_indirect field in DS03, which now means rows like this exist: EOC = labor & is_indirect = Y.
		For our purposes, i.e. joining to DS06, we treat this as a row where EOC = Indirect. 
		To do this, use a CASE: CASE WHEN ISNULL(is_indirect,'') = 'Y' THEN 'Indirect' ELSE EOC END

		This test looks for cost data with no accompanying resources (DS06).

		Step 1. Create CTE, Resources, that joins DS04 Tasks to their DS06 Resources with budget.
		Join by task ID and subproject ID, and group by DS04 WBS ID & DS06 EOC. 
