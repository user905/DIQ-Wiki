...
(
	/*
		...
		If this is the case, it returns a list of CA rows with Actuals.

		The first step is to collect data into two ctes: 
		1. One with A at the CA level (WP ID IS NULL) by CA ID 
		2. One with A at the WP level (WP ID IS NOT NULL) by WP ID
	*/
	...
	/*
		Then filter in the where, looking for COUNT > 0 in both CTEs
		and, if they exist, returning all rows with ACWP.
	*/
	SELECT 
		* 
	FROM 
