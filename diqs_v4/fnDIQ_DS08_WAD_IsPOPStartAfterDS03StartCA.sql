...
(
	...
	/*
		This function looks for CA WADs where pop start > the first recorded DS03.SPAE value.

		First, the CostStart cte gets loaded with the first recorded SPAE value by CA WBS ID.

		Then it's joined back to DS08 and a filter applied comparing pop start to the 
		CostStart period.

		Use LatestCAWADRev_Get to compare only to the latest revision.
	*/
